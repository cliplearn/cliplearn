...
def periodic_cleanup():
    ...
    try:
        ...
        if os.path.exists(AUDIO_DIR):
            cutoff = now - AUDIO_CLEANUP_AGE_DAYS * 86400
        ...
        # 清理过期回收站记录
        conn = get_db_connection()
        cutoff = now - MAX_TRASH_AGE_DAYS * 86400
        conn.execute("DELETE FROM trash_records WHERE trashed_at < ?", (cutoff,))
        conn.commit()
        conn.close()
    finally:
